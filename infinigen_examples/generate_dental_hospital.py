...
# Authors: Dental hospital scene generation script for large outpatient clinics
...
from infinigen_examples.constraints.dental_hospital_floorplan import (
    dental_hospital_floorplan,
)
...
try:
    ...
    from infinigen_examples import (
        generate_nature,  # noqa: F401 # fallback when run as a script
    )
...
@gin.configurable
def compose_dental_hospital(output_folder: Path, scene_seed: int, **overrides):
    ...
    from infinigen_examples.constraints.dental_hospital_furniture_constraints import (
        dental_hospital_furniture_constraints,
    )
    from infinigen_examples.constraints.dental_hospital_room_constraints import (
        dental_hospital_room_constraints,
    )

    consgraph = dental_hospital_furniture_constraints()
    consgraph_rooms = dental_hospital_room_constraints()
    constants = consgraph_rooms.constants

    stages = dental_greedy_stages()
    ...
    solver = Solver(
        ...
        floor_plan=dental_hospital_floorplan,
    )
    ...
    p.run_stage("lights_off", turn_off_lights, use_chance=False)
    ...
    if overrides.get("topview", False):
        ...
        for cam_dist in np.exp(np.linspace(1.0, 5.2, 600)):
            camera.location = (
...
def main(args):
    ...
    init.apply_gin_configs(
        configs=["dental_hospital.gin"] + args.configs,
    ...
    execute_tasks.main(
        compose_scene_func=compose_dental_hospital,
