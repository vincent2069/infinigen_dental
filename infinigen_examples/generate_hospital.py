...
# Authors: Hospital scene generation script for generating large hospital environments
#
# 用法 (Usage):
...
#
# 场景说明:
# - 等候区 (Waiting Room): 大型中央等候区域，配备长椅和椅子
# - 导诊区 (Reception): 信息台，为病人答疑解惑
# - 治疗区 (Treatment Zone): 10-20 个诊室，沿走廊排列
#   - 普通诊室 (Standard Clinic): 基本诊疗室
#   - VIP 诊室 (VIP Clinic): 高端诊疗室，配备检查床
#   - 诊室入口连接 corridor，排列成 2-3 排
...
def hospital_greedy_stages():
    """Returns descriptions of what will be covered by each greedy stage of the solver.

    Modified from default_greedy_stages for hospital-specific furniture placement.
    """
...
@gin.configurable
def compose_hospital(output_folder: Path, scene_seed: int, **overrides):
    """Main hospital scene composition function.

    Args:
        output_folder: Where to save the generated scene
    ...
    # Terrain (optional for indoor hospital)
    ...
    # Import hospital-specific constraints
    from infinigen_examples.constraints.hospital_furniture_constraints import (
        hospital_furniture_constraints,
    )
    from infinigen_examples.constraints.hospital_room_constraints import (
        hospital_room_constraints,
    )
    from infinigen_examples.constraints.hospital_semantics import (
        hospital_asset_usage,
    )

    consgraph = hospital_furniture_constraints()
    consgraph_rooms = hospital_room_constraints(
        num_clinics=overrides.get("num_clinics", 15),
        num_vip_clinics=overrides.get("num_vip_clinics", 3),
    )
    constants = consgraph_rooms.constants

    stages = hospital_greedy_stages()
    ...
    def solve_stage_name(stage_name: str, group: str, **kwargs):
        ...
        for i, vars in enumerate(assignments):
            solver.solve_objects(
                ...
                stages[stage_name],
                vars,
                n_steps=overrides.get(f"solve_steps_{group}", 200),
...
def main(args):
    ...
    # Import hospital constraint modules to register gin configurables
    # 导入医院约束模块以注册 gin 可配置函数
    from infinigen_examples.constraints.hospital_furniture_constraints import (
        hospital_furniture_constraints,
    )
    from infinigen_examples.constraints.hospital_room_constraints import (
    ...
    init.apply_gin_configs(
        configs=["hospital.gin"] + args.configs,
    ...
    execute_tasks.main(
        compose_scene_func=compose_hospital,
